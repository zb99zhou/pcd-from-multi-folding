...
impl<C: Group> TranscriptReprTrait<C> for AllocatedIOPProverMessage<C> {
  fn to_transcript_nums<CS: ConstraintSystem<C::Base>>(
    &self,
    _cs: CS,
  ) -> Result<Vec<AllocatedNum<C::Base>>, SynthesisError> {
...
impl<G: Group> AllocatedIOPVerifierState<G> {
  ...
  fn verify_round_and_update_state<CS: ConstraintSystem<<G as Group>::Base>>(
    ...
    let challenge = transcript.squeeze(cs.namespace(|| "calc challenge"), b"Internal round")?;
  ...
  fn check_and_generate_subclaim<CS: ConstraintSystem<<G as Group>::Base>>(
    ...
    let mut expected_vec = self
      ...
      .map(|(i, (evaluations, challenge))| {
        ...
        enforce_interpolate_uni_poly(
          cs.namespace(|| format!("{i}th interpolate")),
          challenge,
          evaluations,
        )
      })
...
pub fn sumcheck_verify<CS: ConstraintSystem<<G as Group>::Base>, G: Group>(
  ...
  for i in 0..aux_info.num_variables {
    ...
    transcript.absorb(
      cs.namespace(|| format!("{i}th round absorbing")),
      b"prover msg",
      prover_msg,
    )?;
  ...
  for (k, thetas) in vec_thetas.iter().enumerate() {
    ...
    for i in 0..ccs_params.q {
      ...
      for j in ccs_params.S[i].clone() {
        prod = prod.mul(cs.namespace(|| format!("prod * thetas[{j}]")), &thetas[j])?;
      }
      lhs = lhs.add(&Num::from(prod).scale(ccs_params.c[i]));
    }
    let gamma_t1 = gamma.pow_constant(cs.namespace(|| "gamma_t1"), mu * ccs_params.t + k)?;
    let acc = gamma_t1
      .mul(cs.namespace(|| "gamma_t1 * e2"), &e2)?
      .mul_lc(cs.namespace(|| "gamma_t1 * e2 * lhs"), lhs)?;
    c_lc = c_lc.add(&Num::from(acc));
  }
...
pub fn enforce_interpolate_uni_poly<CS: ConstraintSystem<F>, F: PrimeField>(
  ...
  for j in 1..len {
    ...
    let eval_at_sub_j = eval_at.sub_constant(cs.namespace(|| "eval_at - j"), allocated_j)?;
  ...
  for i in (0..len).rev() {
    ...
    let res = p_i[i]
      .mul_with_scale(
        cs.namespace(|| "p_i[i] * prod"),
        &prod,
        F::from(ratio_denominator),
      )?
  ...
  let final_res = AllocatedNum::alloc(cs.namespace(|| "alloc tmp"), || {
